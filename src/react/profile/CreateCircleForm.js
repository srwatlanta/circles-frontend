...
class CreateCircleForm extends Component {
  ...
  fetchAllUsers = () => {
    ...
    fetch("https://circles-backend.herokuapp.com/users", reqObj)
      .then(res => res.json())
      .then(data =>
        this.setState({
          users: data
    ...
    return this.filterUsers().map(user => {
      return (
        <NewCircleAddUser
          ...
          handleMinusClick={this.subtractUserClick}
          handleAddClick={this.addUserClick}
          key={uuid()}
          user={user}
        />
      );
